...
testTsV <- function(m1,m2,v1,v2){
  generate_t_distribution <- function(n, df=3, d, m,v) {
    #mu <- matrix(0, nrow = 1, ncol = d)  # Mean vector (1 component, d dimensions)
    mu <- rep(m,d)
    sigma <- diag(d)*v  # Covariance matrix (identity matrix for no covariance)
    w= c(1)
  ...
  for (d in dimensions) {
    ...
    for (i in 1:replications) {
      # Generate data
      data1 <- generate_t_distribution(sample_size, df = 3, d,m1,v1)
      data2 <- generate_t_distribution(sample_size, df = 3, d,m2,v2)
...
rest221 = testTsV(0,0,1,10^0.01)
rest222 = testTsV(0,0,1,10^0.02)
rest223 = testTsV(0,0,1,10^0.03)
rest224 = testTsV(0,0,1,1)
